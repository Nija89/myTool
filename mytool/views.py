...
def add_task(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, 'success.html', context)  # Render the success page
        else:
            # If the form is not valid, show the form again with errors
            print(form.errors)
            return render(request, 'add_task.html', {'form': form})  # Show the form again for corrections

    else:
        form = addTask()  # Create a new empty form

    return render(request, 'add_task.html', {'form': form})  # Render the form page initially